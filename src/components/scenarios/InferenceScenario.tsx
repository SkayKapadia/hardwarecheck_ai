...
import { ModelPicker, getModelById, getCustomModelMeta, registerCustomModel } from "@/components/ModelPicker";
...
import { Zap, Layers, ArrowRight, RotateCcw, ChevronDown, ChevronUp, CheckCircle2, XCircle } from "lucide-react";
...
export function InferenceScenario() {
  ...
  const memoryRange = getMemoryRange(totalWithSafety);

  const primaryGpu = selectedGPUsData[0];
  const gpuCount = selectedGPUsData.length;
  const totalVram = selectedGPUsData.reduce((sum, g) => sum + g.vram, 0);
  const minVramGpu = selectedGPUsData.reduce((min, g) => (g.vram < min.vram ? g : min), selectedGPUsData[0]);

  // CPU offload split: whole layers move to system RAM once the VRAM left
  // after non-weight allocations runs out.
  const layerSize = rawWeights / selectedModelData.layers;
  const vramForWeights = primaryGpu
    ? Math.max(0, primaryGpu.vram - reserve - kvPerCard - metadata - activations)
    : 0;
  const { cpuLayers } = store.cpuOffload
    ? getCPUSplit(selectedModelData.layers, vramForWeights, layerSize)
    : { cpuLayers: 0 };
  const offloadRatio = cpuLayers / selectedModelData.layers;
  const cpuPart = rawWeights * offloadRatio;
  const gpuPart = totalWithSafety - cpuPart;

  let doesFit = false;
  if (primaryGpu) {
    if (store.cpuOffload) {
      doesFit = gpuPart <= primaryGpu.vram && cpuPart <= store.systemRam;
    } else if (gpuCount > 1) {
      // Tensor parallelism needs the per-card footprint to fit the smallest card.
      doesFit = totalWithSafety <= minVramGpu.vram;
    } else {
      doesFit = totalWithSafety <= primaryGpu.vram;
    }
  }

  const tps = primaryGpu
    ? estimateTPS(
        selectedModelData,
        primaryGpu,
        store.quantization,
  ...
  const gpuListLabel = (() => {
    ...
    for (const g of selectedGPUsData) {
      const entry = counts.get(g.id) ?? { name: g.name, count: 0 };
      entry.count += 1;
      counts.set(g.id, entry);
    }
    return Array.from(counts.values())
      .map((e) => (e.count > 1 ? `${e.count}× ${e.name}` : e.name))
      .join(" + ");
  })();

  const customMeta = getCustomModelMeta(selectedModelData.id);
      ...
      <div className="w-[45%] flex flex-col gap-6 overflow-y-auto pr-4 custom-scrollbar pb-10">
        ...
        <div className="space-y-3">
          <label className="text-xs font-mono text-muted-foreground uppercase tracking-widest flex items-center gap-2">
            AI Model <InfoPopup content="Select the open-weight model to evaluate, or import one from Hugging Face." />
          </label>
          <HuggingFaceImport
            onImport={(spec, meta) => {
              registerCustomModel(spec, meta);
      ...
      <div className="w-[55%] flex flex-col gap-6 overflow-y-auto pb-10 custom-scrollbar pr-2">
        ...
        <div className={`p-6 rounded-xl border flex items-start gap-4 ${
          ...
          <div>
            ...
            <p className="text-muted-foreground leading-relaxed">
              <span className="text-foreground font-semibold">{selectedModelData.name}</span> at{" "}
              <span className="text-foreground font-semibold">{store.quantization}</span> is estimated to need{" "}
              {store.cpuOffload && primaryGpu ? (
                <>
                  <span className="text-foreground font-semibold">≈{totalWithSafety.toFixed(1)} GB</span>: up to{" "}
                  <span className="text-foreground font-semibold">{gpuPart.toFixed(1)} GB</span> on the{" "}
                  {primaryGpu.name} ({primaryGpu.vram} GB VRAM) +{" "}
                  <span className="text-foreground font-semibold">{cpuPart.toFixed(1)} GB</span> offloaded to system
                  RAM (<span className="text-foreground font-semibold">{store.systemRam} GB</span>).
                </>
              ) : gpuCount > 1 ? (
                <>
                  <span className="text-foreground font-semibold">≈{totalWithSafety.toFixed(1)} GB per card</span>{" "}
                  across {gpuListLabel} (<span className="text-foreground font-semibold">{totalVram} GB</span>{" "}
                  combined). The smallest card ({minVramGpu.name}, {minVramGpu.vram} GB) is the binding constraint.
                </>
              ) : (
                <>
                  <span className="text-foreground font-semibold">≈{totalWithSafety.toFixed(1)} GB</span> of the{" "}
                  {primaryGpu?.name ?? "GPU"}'s{" "}
                  <span className="text-foreground font-semibold">{primaryGpu?.vram ?? 0} GB</span> VRAM.
                </>
              )}
            </p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 border-b border-border pb-6">
          ...
          <div className="text-center border-r border-border">
            ...
            <div className="font-semibold text-foreground">Supported • {getRuntimeForQuant(store.quantization)}</div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="border border-border rounded-xl bg-card/30 p-6 space-y-8">
            <div className="flex justify-between text-xs font-mono text-muted-foreground border-b border-border/50 pb-2">
              ...
              <div>
                {selectedModelData.activeParams
                  ? `${formatParams(selectedModelData.params)} total / ${formatParams(selectedModelData.activeParams)} active`
                  : `${formatParams(selectedModelData.params)} params`}
              </div>
            ...
            {selectedModelData.activeParams && (
              <div className="text-[10px] font-mono text-muted-foreground -mt-6">
                MoE: memory is computed from total params, speed from active params.
              </div>
            )}
            ...
            <div className="text-[10px] font-mono text-muted-foreground border-t border-border/50 pt-2">
              {selectedModelData.family === "Custom" ? (
                <>
                  unverified — imported from Hugging Face ({selectedModelData.source})
                  {customMeta?.paramsEstimated && " · params estimated from config"}
                </>
              ) : (
                <>
                  verified {selectedModelData.verifiedOn}
                  {selectedModelData.source && ` · source: ${selectedModelData.source}`}
                </>
              )}
            </div>
          </div>
        ...
        <div className="space-y-4 pt-4">
          <div className="flex justify-between text-xs font-mono text-muted-foreground uppercase tracking-widest">
            <span>0 GB</span>
            <span>
              ≈ {totalWithSafety.toFixed(1)} GB ({memoryRange.low.toFixed(1)}–{memoryRange.high.toFixed(1)} depending on runtime) TOTAL REQUIRED
            </span>
          </div>
